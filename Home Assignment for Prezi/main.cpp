...
bool right_rotate(Point p1, Point p2, Point p3) {
    return p1.x * (p2.y - p3.y) + p2.x * (p3.y - p1.y) + p3.x * (p1.y - p2.y) < 0;
}

bool left_rotate(Point p1, Point p2, Point p3) {
    return p1.x * (p2.y - p3.y) + p2.x * (p3.y - p1.y) + p3.x * (p1.y - p2.y) > 0;
}
...
public:
    ...
    void eliminate_duplicates() {
        set<Point> s(intersects.begin(), intersects.end());
        intersects.assign(s.begin(), s.end());
    }
    ...
    void convex_hull(vector<Point> &points) {
        ...
        for(size_t i = 1; i<points.size(); i++) {
            if(i == points.size()-1 || right_rotate(p1, points[i], p2)) {
                while (up.size()>=2 && !right_rotate(up[up.size()-2], up[up.size()-1], points[i]))
                    up.pop_back();
                up.push_back(points[i]);
            }
            if(i == points.size()-1 || left_rotate(p1, points[i], p2)) {
                while (down.size()>=2 && !left_rotate(down[down.size()-2], down[down.size()-1], points[i]))
                    down.pop_back();
    ...
    void print_result() {
        ...
        printf("%.4f", convex_hull_area(convex_hull_points));
    }
};
